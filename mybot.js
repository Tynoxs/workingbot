...
client.on('message', message => {  
	...
	message.channel.sendMessage(`You have been muted for ${ms(ms(time), {long: true})} ${member.user.tag}`);
	
	setTimeout(function() {
		member.removeRole(mute.id);
		message.channel.sendMessage(`${member.user.tag} you have been unmuted, please behave.`);
	}, ms(time));
	}
